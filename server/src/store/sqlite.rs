//! The self-host backend: one file, one binary, no second container.
...
impl SqliteStore {
    pub async fn connect(url: &str) -> Result<Self> {
        let options: SqliteConnectOptions = url
            .parse()
            .map_err(|e| Error::Internal(format!("bad sqlite url: {e}")))?;
        let options = options
        ...
        let pool = SqlitePoolOptions::new()
            // SQLite serialises writers anyway; a large pool just moves the contention.
            .max_connections(4)
...
impl Store for SqliteStore {
    async fn migrate(&self) -> Result<()> {
        sqlx::raw_sql(include_str!("schema_sqlite.sql"))
            .execute(&self.pool)
            .await?;
        Ok(())
    }
    ...
    async fn consume_token(&self, hash: &str, kind: &str) -> Result<Option<TokenRow>> {
        // Single-use: deleting as we read is what stops a stolen refresh token being
        // replayed alongside the legitimate one. RETURNING makes it one statement, so
    ...
    async fn put_envelope(&self, account_id: &str, envelope: &Envelope) -> Result<Option<i64>> {
        ...
        let seq: i64 =
            sqlx::query("UPDATE sequence SET next = next + 1 WHERE id = 1 RETURNING next")
                .fetch_one(&mut *tx)
                .await?
                .get("next");
        ...
        // The last-write-wins comparison lives in the WHERE of the upsert, so it happens
        // under the same lock as the write. Doing it as read-then-write in Rust would let
        // two devices pushing at once interleave into a lost update.
        let row = sqlx::query(
        ...
        tx.commit().await?;
        // A burnt sequence number when the write loses is fine: the cursor only ever
        // moves forward through `seq > n`, so gaps cost nothing.
    ...
    async fn max_seq(&self, _account_id: &str) -> Result<i64> {
        let row = sqlx::query("SELECT next FROM sequence WHERE id = 1")
            .fetch_one(&self.pool)
            .await?;
        Ok(row.get("next"))
    }
...
fn account_from_row(r: sqlx::sqlite::SqliteRow) -> Account {
    Account {
        id: r.get("id"),
        email: r.get("email"),
...
fn envelope_from_row(r: sqlx::sqlite::SqliteRow) -> Result<Envelope> {
...
fn new_id() -> String {
    // No uuid crate here: 16 random bytes as hex is the same thing for this purpose and
    // one fewer dependency on the server.
    use rand::Rng as _;
...
fn share_from_row(r: sqlx::sqlite::SqliteRow) -> ShareRow {
    ShareRow {
        group_id: r.get("group_id"),
